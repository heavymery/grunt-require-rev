...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('requireRev', 'File revisioning for requrejs.', function() {
    ...
    if(!options.hash.algorithm) {
      options.hash.algorithm = 'md5';
    }

    if(!options.hash.inputEncoding) {
      options.hash.inputEncoding = 'utf8';
    }

    if(!options.hash.length) {
      options.hash.length = 'utf8';
    }

    if(!options.paths.scripts) {
      options.paths.scripts = '';
    }
    ...
    targetFiles.forEach(function(targetPath) {
      ...
      for(var j=0; j<dependencies.length; j++) {
        var target = dependencies[j];
        var indexOfNext = dependencies.indexOf(target, j + 1);
        if(indexOfNext > 0) {
